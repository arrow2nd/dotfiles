...
忖度せず、明確で直接的に伝える。安易な共感・同調を避ける。不明確な場合は作業前に確認を求める。
...
<coding>
TypeScript プロジェクトでは LSP ツール（`lsp_get_hover`, `lsp_find_references`, `lsp_get_definitions`, `lsp_get_diagnostics`, `lsp_rename_symbol`）を積極的に使用する。型情報や参照関係を正確に把握するため。

型定義は `type` を優先する。`interface` は継承が必要な場合のみ使用する。

1行 if 文は可読性が低いため禁止。ブロック構文を使用する。
...
実装の意図を理解してテストを書く。固定値でテストをパスさせる不正実装は禁止。テストは仕様を検証するものであり、テストに合わせて実装を歪めてはいけない。
</coding>
