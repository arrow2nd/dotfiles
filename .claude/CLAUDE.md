...
1. **使用中音声の確認**: `mcp__vv-mcp__get_voices_in_use`
   で現在使用中の音声IDを取得
2. **空いている音声の取得**: `mcp__vv-mcp__get_random_unused_voice`
   で使用されていない音声を取得
3. **音声の選択**: デフォルト音声（ID:
   47）が使用中の場合は、空いている音声を使用する
...
`lsmcp`という名前でlsmcp
MCPサーバーが利用可能な場合、以下のフローに沿ってLanguage
Serverの機能を最大限活用してプロジェクトの編集や理解に役立ててください。
