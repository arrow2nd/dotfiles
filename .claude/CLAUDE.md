...
TypeScriptプロジェクトでの編集作業において、型チェックやLanguage
Server機能を活用してください。

### 使用場面

TypeScriptプロジェクトでの作業時は、以下の場面で**必ずlsmcpを活用**してください：

1. **ファイル編集前の診断**
   - `mcp__ide__getDiagnostics`ツールでエラーや警告を確認
2. **エラー解消作業**
   - 型エラーやESLintエラーの修正時にlsmcpで詳細な診断情報を取得
3. **リファクタリング**
   - 型安全性を保ちながらコードを変更する際の支援
4. **新機能実装**
   - TypeScriptの型システムを活用した開発支援

### 使用フロー

```
1. mcp__ide__getDiagnostics でエラー・警告を確認
2. 必要に応じてファイルを編集
3. 再度診断を実行して修正結果を確認
```
...
- TypeScriptプロジェクトでの作業では、手動でのエラー推測ではなく、必ずlsmcpの診断結果を基に作業してください
- エラー修正後は必ず診断を再実行して、問題が解決されていることを確認してください
